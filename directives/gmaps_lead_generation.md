...
This workflow scrapes Google Maps for local businesses, enriches them with contact info from their websites, and outputs structured leads to Google Sheets.
...
### Installation
```bash
pip install apify-client httpx html2text anthropic gspread google-auth python-dotenv
```
...
### Quick One-Liner
```bash
python3 execution/gmaps_lead_pipeline.py --search "[QUERY]" --limit 50
```
