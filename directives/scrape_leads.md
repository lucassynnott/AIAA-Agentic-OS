...
This workflow scrapes leads using Apify, verifies industry relevance with AI, and saves verified leads to Google Sheets with parallel processing support.

## What It Does
1. Scrapes leads via Apify actor
2. AI verifies industry match (>80%)
3. Filters and enriches leads
4. Saves to Google Sheet
5. Supports parallel scraping for scale
...
### Installation
```bash
pip install apify-client openai gspread google-auth
```
...
### Basic Scrape
```bash
python3 execution/scrape_leads.py \
  --query "SaaS companies Austin" \
  --industry "software" \
  --limit 100 \
...
```bash
python3 execution/scrape_leads.py --query "[QUERY]" --limit 1000 --parallel 5
```

### Quick One-Liner
```bash
python3 execution/scrape_leads.py --query "[QUERY]" --limit 100
```
