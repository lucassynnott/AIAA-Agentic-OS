...
This workflow scrapes Google search results for local businesses, fetches websites, extracts 100+ contact fields using GPT, and stores leads in Google Sheets.

## What It Does
1. Searches Google via Apify actor
2. Fetches each result URL
3. GPT extracts 100+ fields per lead
4. Generates confidence scores
5. Creates outreach icebreakers
...
### Via N8N Webhook
```
GET https://[YOUR_N8N_URL]/webhook/[WEBHOOK_ID]?query=calgary+plumber
```

### Via Python Script
```bash
python3 execution/serp_lead_scraper.py \
  --query "calgary plumber" \
  --country ca \
  --pages 5 \
...
### Quick One-Liner
```bash
python3 execution/serp_lead_scraper.py --query "[QUERY]" --pages 3
```
