...
This workflow scrapes Upwork jobs matching keywords, generates AI-personalized cover letters, and outputs to Google Sheets with apply links.

## What It Does
1. Scrapes Upwork for matching jobs
2. Filters by budget, skills, time
3. Generates personalized cover letters
4. Creates proposal with pricing
5. Outputs to Sheet with apply links
...
### Installation
```bash
pip install openai gspread google-auth requests
```

## How to Run
```bash
python3 execution/upwork_scrape_apply.py --keywords "AI,automation,n8n" --min_budget 500
```
